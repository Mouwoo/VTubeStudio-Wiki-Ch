...
![NVIDIA Broadcast 完整标识](img/img/nvidia_broadcast_logo_full.png)

![RTX ON 小型标识](img/img/rtx_on_smol.png)
...
![NVIDIA 跟踪器演示动图](img/img/nvidia_tracker_example.gif){: height="" width="800px"}
...
[![RTX DLC 下载界面](img/img/rtx_dlc_download.png){: height="" width="auto"}](https://store.steampowered.com/app/2178540/VTube_Studio__NVIDIA_Broadcast_Tracker)

跟踪器文件相对较大（~2 GB），因此请确保你的硬盘上有足够的空间。

在启动 VTube Studio 之前，请确保 DLC 已下载并安装（在 DLC 窗口中设置复选标记）：

![RTX 跟踪器 DLC 安装确认界面](img/img/rtx_tracker_dlc_installed.png)

## 如何使用？

NVIDIA Broadcast 跟踪器可以像常规 OpenSeeFace 网络摄像头跟踪器一样启动。单击 **"跟踪类型"**按钮后，选择 **"RTX - NVIDIA Broadcast"**（参见 1）。仅当你实际拥有支持 RTX 的 GPU 时，此功能才可用。

![RTX 跟踪器 UI 说明图](img/img/rtx_ui_explanation.png)
...
![NVIDIA 跟踪器混合形状微调界面](img/img/nvidia_tracker_finetuning.png)
...
2.  **确保"弱光补偿"已关闭**
    ...
    ![跟踪器摄像头配置窗口](img/img/tracker_webcam_config_win.png){: height="" width="490px"}

    这将打开以下窗口：（确保弱光补偿已关闭）

    ![弱光补偿设置界面](img/img/low_fps_low_light_compensation.png){: height="" width="578px"}

3.  **确保"抗闪烁"设置为 60 Hz**
    如果"防闪烁"设置为 50 Hz，则你的相机帧速率上限为 50 FPS。

    ![抗闪烁设置界面](img/img/low_fps_anti_flicker.png){: height="" width="578px"}
...
跟踪器通过你的本地网络将跟踪数据发送到 VTube Studio，因此你的 Windows 防火墙可能会阻止它。确保添加 `"ExpressionApp.exe"` 为防火墙例外。
...
![防火墙配置示例图](img/img/firewall_config.png)
